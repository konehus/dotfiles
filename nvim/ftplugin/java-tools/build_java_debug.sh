#!/bin/bash
# Clone and build microsoft/java-debug with optional -u (force update)
...
TARGET_DIR="$ORG_DIR/java-debug"
VERSION_FILE="$TARGET_DIR/version.txt"
REPO_URL="https://github.com/microsoft/java-debug.git"

mkdir -p "$ORG_DIR"

# Clone if not exists
if [ ! -d "$TARGET_DIR/.git" ]; then
  echo -e "${BLUE}${ICON_CLONE} Cloning microsoft/java-debug into ${YELLOW}$TARGET_DIR${NC}"
...
if [ "$FORCE_UPDATE" -eq 0 ]; then
  if [ "$STORED_VERSION" == "$LATEST_COMMIT" ]; then
    echo -e "${GREEN}${ICON_SUCCESS} java-debug is up-to-date at commit ${YELLOW}$LATEST_COMMIT${NC}"
    exit 0
  else
...
# Build
echo -e "${BLUE}${ICON_BUILD} Building java-debug with ./mvnw clean install...${NC}"
(cd "$TARGET_DIR" && ./mvnw clean install)
...
echo -e "${GREEN}${ICON_SUCCESS} java-debug updated and built at commit ${YELLOW}$LATEST_COMMIT${NC}"
